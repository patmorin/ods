...
A #ScapegoatTree# that keeps itself balanced by \emph{partial rebuilding
...
\begin{figure}
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption[Adding to a scapegoat tree]{Inserting 3.5 into a #ScapegoatTree# increases its depth to 6, which violates \myeqref{scapegoat-height} since $6 > \log_{3/2} 11 \approx 5.914$.  A scapegoat is found at the node containing 5.}
\end{figure}
...
\begin{exc}
  ...
  to first connect the nodes into a linked list and the convert this
  linked list into a perfectly balanced binary tree.
\end{exc}
...
\begin{exc}
  Analyze and implement a #WeightBalancedTree2#.  This is a tree in
  which each node #u#, except the root, maintains the \emph{balance
  invariant} that $#size(u)# \le (2/3)#size(u.parent)#$.  The #add(x)# and
  #remove(x)# operations are identical to the standard #BinarySearchTree#
  operations, except that any time the balance invariant is violated at
  ...
  Your analysis should show that operations on a #WeightBalancedTree2#
