...
size #n#, i.e. the element which is greater than #x# and less than
$#n#-#x#-1$ elements in the set.
...
\begin{lem}\lemlabel{rbs}
  ...
  \begin{enumerate}
    ...
    the search path for #x# is $H_{#x#+1} + H_{#n#-#x#-1} - O(1)$.\footnote{It may be useful to interpret the expressions $#x#+1$ and $#n#-#x#-1$ as the number of elements in the tree less than or equal to #x# and the number of elements in the tree greater than #x# respectively.}
    \item For any $#x#\in\{-1,\ldots,#n#-1\}$, the expected length of the
    search path for $#x#+1/2$ is $H_{#x#+1} + H_{#n#-#x#-1}$.
  \end{enumerate}
\end{lem}
...
that if we search for the element of rank #x# in a tree of size #n#, then the expected
...
tree; here, $#x#+1/2$ stands in for a value between the element of
rank #x# and its successor in the tree or for a lower or upper bound
on the elements in the tree (when #x# is $-1$ or $n-1$ respectively).
...
To see this, refer to \figref{rbst-records} and notice that, until some value in $\{i,i+1,\ldots,#x#\}$
