...
\begin{itemize}
  ...
  \item Log in to your favourite social network:  The network servers
...
\subsection{The Need for Efficiency}
...
\paragraph{Bigger data sets:} Now consider a company like Google, that
...
An \emph{interface}, sometimes also called an \emph{abstract data
...
A \emph{FIFO (first-in-first-out) #Queue#}, which is illustrated in
...
so the qualifier FIFO is often ommitted.  In other texts, the #add(x)#
and #remove()# operations on a FIFO #Queue# are often called #enqueue(x)#
and #dequeue()#, respectively.

\begin{figure}
  \centering{\includegraphics[scale=0.90909,width=\ScaleIfNeeded]{figs/queue}}
  \caption[A FIFO queue]{A FIFO #Queue#.}
  \figlabel{queue}
\end{figure}

A \emph{priority #Queue#}, illustrated in \figref{prioqueue}, always
...
\begin{figure}
  \centering{\includegraphics[scale=0.90909,width=\ScaleIfNeeded]{figs/prioqueue}}
...
\begin{figure}
  \centering{\includegraphics[scale=0.90909,width=\ScaleIfNeeded]{figs/stack}}
...
#List# interface.  A #List#, illustrated in \figref{list}, represents a
...
\begin{figure}
  \centering{\includegraphics[scale=0.90909,width=\ScaleIfNeeded]{figs/list}}
...
random variables}.  These binary variables are useful when we want to
...
the coin turns up as heads.  Intuitively, we know the answer is $k/2$,
...
word-RAM} model.  RAM stands for Random Access Machine. In this model, we
have access to a random access memory consisting of \emph{cells}, each of
which stores a #w#-bit \emph{word}.  This implies that a memory cell can
...
to the newly-allocated memory block.  This reference is small enough to
...
\begin{figure}
  \begin{center}
    \includegraphics[scale=0.90909,width=\ScaleIfNeeded]{figs/dependencies}
  \end{center}
