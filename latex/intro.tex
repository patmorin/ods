...
discipline.  In a \emph{LIFO Queue}, the most recent element added is the
...
in place of $5n\log_2 n + 8n - 200$ we can write, simply,  $O(n\log_2 n)$.
This is proven as follows:
\begin{align*} 
       5n\log_2 n + 8n - 200
        & \le 5n\log_2 n + 8n \\
        & \le 5n\log_2 n + 8n\log_2 n & \mbox{ for $n\ge 2$ (so that $\log_2 n \ge 1$)}
            \\
        & \le 13n\log_2 n 
\end{align*}
which demonstrates that $5n\log_2 n + 8n - 200$ is in $O(\log_2 n)$ using the constants $c=13$ and $n_0 = 2$.
...
\binom{k}{i}/2^k$ and that we know the binomial identity $\sum_{i=0}^{k}
\binom{k}{i} = 2^{k}$.
...
up heads precisely because each coin has probability $1/2$ of coming
up heads.
