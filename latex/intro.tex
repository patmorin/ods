...
In the next section, we look at the operations supported by the
...
These examples suggest that operations which require looking at everything
...
It may seem counterintuitive, but we will see data structures where
a search requires looking at only 2 items on average, independent
of the number of items stored in the data structure.  In our billion
instruction per second computer, these operations take $0.000000002$
seconds to do search an #SSet# containing a billion items (or a trillion,
or a quadrillion, or even a quintillion items).

We will see implementations of data structures that keep the items
in sorted order, where the number of items inspected during an
