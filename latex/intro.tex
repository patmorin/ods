...
For a non-negative integer #n#, the notation $#n#!$ denotes
\[
...
exactly, we will use the so-called big-Oh notation: For a 
...
memory of size $k$ takes $O(k)$ time and returns a reference to a memory
...
\multicolumn{4}{|c|}{#SSet# implementations} \\ \hline
 & #find(x)# & #add(x)#/#remove(x)# & \\ \hline
#SkiplistSet# & $O(\log #n#)$\footnotemark[5] & $O(\log #n#)$\footnotemark[5] & \sref{skiplistset} \\ \hline
...
#MeldableHeap# & $O(1)$ & $O(\log #n#)$\footnotemark[5] & \sref{mergeableheap} \\ \hline
\end{tabular}
