...
\begin{itemize}
  \item #min()#: returns the minimum value currently stored in the data structure.
\end{itemize}
...
\begin{exc}
  ...
  comparable elements and supports the queue operations #addFirst(x)#, #addLast(x)#
  #removeFirst()#, #removeLast()# and #size()#, as well as the following operation:
\begin{itemize}
  \item #min()#: returns the minimum value currently stored in the data structure.
\end{itemize}
  All operations should run in constant amortized time.
