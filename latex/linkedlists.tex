...
and #u.next#.  The list itself contains two pointers, one to the #dummy#
...
This technique complicates the code a little and is not possible in some
garbage-collected languages\javaonly{---including Java---}\cpponly{
}but gives a doubly-linked list implementation that requires only one
pointer per node.  A detailed discussion of XOR-lists can be found in
Sinha's magazine article \cite{s04}.
