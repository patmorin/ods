...
Se temos uma referência a um nodo #w# em uma #DLList# e queremos inserir um nodo #u# antes de #w#, então isso é só uma questão de atribuir 
 $#u.next#=#w#$,
$#u.prev#=#w.prev#$, e então ajustar #u.prev.next# e  #u.next.prev#.  (Ver \figref{dllist-addbefore}.)
...
descrita em \secref{arraydeque}. A #BDeque# difere do #ArrayDeque#
...
o limitante inferior $\sqrt{#n#}$ descrito em \secref{rootishspaceusage}.
...
nossa exploração (ver \figref{selist-add}):
...
a #spread(u)# e #gather(u)#, uma #SEList# tamanho de bloco #b#
  ...
  \footnote{Consulte \secref{model} para uma discussão de como a memória é medida.
...
\begin{exc}\exclabel{dllist-sort}
  ...
  merge-sort para a ordenação de uma
   #DLList#, conforme discutido em \secref{merge-sort}.
  ...
  qualquer #DLList# contendo elementos que implementam da interface #Comparable#.}
  \begin{enumerate}
