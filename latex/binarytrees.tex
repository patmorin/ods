...
A #BinarySearchTree# implements the #SSet# interface. A #BinarySearchTree# supports
...
#n#)$ expected time per operation.  The problem with the #BinarySearchTree#
structure is that it can become \emph{unbalanced}.  Instead of looking
like the tree in \figref{bst}, it can look like a long chain of
#n# nodes, all but the last having exactly one child.
...
In \chapref{24redblack} we show how $O(\log #n#)$ \emph{worst-case} time
