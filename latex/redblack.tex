...
\begin{exc}\exclabel{redblack-merge}
  Suppose you have two AVL trees $T_1$ and $T_2$ that have the same black
  height, $h$, and that the largest key in $T_1$ is smaller than the
  smallest key in $T_2$.  Show how to merge $T_1$ and $T_2$ in into a
  single red-black tree in $O(h)$ time.
\end{exc}
