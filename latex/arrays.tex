...
\begin{figure}
  ...
  \end{center}
  \caption{A sequence of #add(i,x)# and #remove(i)# operation in an
...
\begin{proof}
  ...
  $#n#/2-1$.  Therefore, if $n_i$ denotes the value of #n# during the $i$th
call to #resize()#, then the total number of calls to #add(i,x)# or
...
array #a#.  An #ArrayQueue# simulates this by using a finite array #a#
...
A sequence of #add(x)# and #remove()# operation on an #ArrayQueue# are illusted in \figref{arrayqueue}.
...
\begin{figure}
  ...
  \end{center}
  \caption{A sequence of #add(x)# and #remove(i)# operation on an
...
removal at both ends.  This structure implements the #List# interface
using the same circular array technique used to represent an #ArrayQueue#.

\javaimportwithclass{ods/ArrayDeque.a.j.n}

The #get(i)# and #set(i,x)# operations in an #ArrayDeque# are
straightforward.  They get or set the array element $#a[#{#(j+i)#\bmod
#a.length#}#]#$.

\javaimport{ods/ArrayDeque.get(i).set(i,x)}

Adding and removing elements from an #ArrayDeque# is illustrated in
\figref{arraydeque}.  The implementation of #add(i,x)# is a little
more interesting.   As usual, we first check if #a# is full and,
if necessary, call #resize()# to resize #a#.   Remember that we want
this operation to be fast when #i# is small (close to 0) or when #i#
is large (close to #n#).  Therefore, we check if $#i#<#n#/2$.  If so,
we shift the elements $#a[0]#,\ldots,#a[i-1]#$ left by one position.
Otherwise ($#i#\ge#n#/2$), we shift the elements $#a[i]#,\ldots,#a[n-1]#$
...
Note that, if a $#i#<#front.size()#$, then this translates to accessing
the element of #front# at position $#front.size()#-#i#-1$, since the
elements of #front# are stored in reverse order.

Adding and removing elements from an #ArrayDeque# is illustrated in
\figref{arraydeque}.  The #add(i,x)# operation manipulates either #front#
...
\begin{figure}
  ...
  \end{center}
  \caption{A sequence of #add(i,x)# and #remove(i)# operation in a
