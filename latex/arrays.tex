...
\javaimport{ods/ArrayStack.a.n.size()}
...
\javaimport{ods/ArrayQueue.a.j.n}
...
\javaimport{ods/ArrayDeque.a.j.n}
...
\javaimport{ods/DualArrayDeque.front.back}
...
\javaimport{ods/RootishArrayStack.blocks.n}
...
can be found in implementations dating back over 30 years.  For example, implementations of stacks, queues, and deques which generalize easily to the #ArrayStack#, #ArrayQueue# and #ArrayDeque# structures described here are discussed by Knuth \cite[Section~2.2.2]{k97v1}.
...
supports #get(i,x)# and #set(i,x)# in constant time and #add(x)# and
#remove(x)# in $O(\sqrt{#n#})$ time.  These running times are similar
