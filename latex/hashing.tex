...
\begin{exc}
  ...
  worked by not using all characters of long strings. For example, for
  a 16 character string, the hash code was computed using only the 8
  even-indexed characters. Explain why this was a very bad idea.  Give an
  example of large set of strings that all have the same hash code.
\end{exc}
...
\begin{exc}
  ...
  timing how long it takes to add and find particular values. (This
  kind of approach can be used to launch denial of service attacks on
  web servers \cite{cw03}.)
\end{exc}
