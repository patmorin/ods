...
Ao longo deste livro, consideremos o modelo de computação RAM com palavras de #w# bits definido na \secref{model}.   Uma premissa implícita desse
...
Cada nodo #u# na $B$-tree guarda um array de chaves 
...
\figref{btree-find}, generaliza a operação #find(x)# em uma árvore binária de busca. A busca por #x# inicia na raiz e usa as chaves guardadas em um nodo #u# para
determinar em qual dos filhos de #u# a busca deve continuar.

\begin{figure}
  \centering{\includegraphics[width=\ScaleIfNeeded]{figs/btree-2}}
  \caption[Busca em uma $B$-tree]{Uma busca bem sucedida (pelo valor 4) e uma busca mal sucedida (pelo valor 16.5) em uma $B$-tree. Nodos sombreados onde o valor de #z# fio atualizado durante as buscas.}\figlabel{btree-find}
\end{figure}
...
Quando um underflow acontece, #u# pode emprestar chaves ou ser juntado com seus irmão. Se #u# é juntado com um irmão, então o pai de #u#
...
O propósito do 
 \lemref{btree-split} é mostrar que, no modelo RAM com palavras de #w# bits, 
...
\begin{thm}[$B$-Tree em RAM com palavras de #w# bits]
...
$B$-trees implementa a interface #SSet#. Se a interface #USet# for necessária, então hashing em memória externa
