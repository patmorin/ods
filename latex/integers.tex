...
Para uma folha em uma trie binária,
 ...
 Um nodo especial, #dummy#, é usado tanto antes do primeiro nodo quanto depois do último nodo na lista (veja a \secref{dllist}).
\cpponly{Nos trechos de código, #u.child[0]#, #u.left#, e #u.prev# referem-se ao mesmo campo no nodo #u#, assim como #u.child[1]#, #u.right# e #u.next#.}
...
A operação #remove(x)# desfaz o trabalho de 
...
\figref{xfast-path}; nessa figura o último nodo #u#, no
...
hash 
(\chapref{hashing}).  Usando esse #USet# nos permite checar em tempo constante se existe um nodo no nível #i# cuja marcação corresponde aos #i# bits de ordem mais alta de #x#. Podemos encontrar esse nodo usando
...
Adicionar #x# a #t# leva
 $O(\log #w#)$ de tempo.  \excref{treap-split} mostra que dividir #t# em #t1# 
...
\excref{treap-split} mostra que a junção de todos os elementos de #t# em #t2#
...
Anteriormente, postergamos a discussão sobre os tamanhos das treaps nesta
